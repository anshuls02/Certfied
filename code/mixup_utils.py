...
def modify_loader(loader, mode, ep=None, n_eps=None):
    class_count = torch.tensor(loader.dataset.get_frequency()) #np.unique(torch.tensor(loader.dataset.targets), return_counts=True)[1]
...
def get_combo_loader(loader, mode='instance'):
    if mode == 'instance':
        imbalanced_loader = loader
    else:
        imbalanced_loader = modify_loader(loader, mode=mode)
